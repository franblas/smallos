...
// US Keyboard Layout scancode table (shifted values)
u8 kbdus_shifted[128] = {
    0,  27, '!', '@', '#', '$', '%', '^', '&', '*',	/* 9 */
  '(', ')', '_', '+', '\b',	/* Backspace */
  '\t',			/* Tab */
  'Q', 'W', 'E', 'R',	/* 19 */
...
void write_key(u8 scancode) {
  if (SHIFT_PRESSED) {
    write_char(kbdus_shifted[scancode]);
  } else {
    write_char(kbdus[scancode]);
  }
}
